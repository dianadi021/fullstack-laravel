...
> [![database/TableRelation.txt]](https://github.com/dianadi021/fullstack-laravel/tree/main/database/TableRelation.txt) </br>
...
> .env
```bash
DB_HOST=host.docker.internal # Jika koneksi DB di localhost namun Apps/Laravel di Docker
```
```bash
DB_HOST=localhost # Jika koneksi DB dan Apps/Laravel di localhost / parent host
```
```bash
DB_HOST=laravel_postgres # Jika koneksi DB di docker
```
```bash
DB_CONNECTION=pgsql
